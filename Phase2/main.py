    ...
    if Knob1ForPipelining:
        ...
        for i in reversed(range(5)):
            ...
            if(i==0):
                ...
                states[i+1]=states[i]
                states[i]=None
# fetch -> branch instruction it is first encountered then master_pc = this.pc+4
# target should have been taken
# it will get eventually updated to the correct PC in decode stage itself
            if(i==1):
                if(states[i]==None):
                    continue
                if(stall == i):
